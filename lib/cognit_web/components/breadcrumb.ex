defmodule CognitWeb.Components.Breadcrumb do
  @moduledoc """
  Provides a flexible and customizable `CognitWeb.Components.Breadcrumb` component for displaying
  ...
  def breadcrumb(assigns) do
    ...
    <nav class={@class} id={@id} {@rest}>
      <ol class={[ default_classes(), color_class(@color), size_class(@size), @items_wrapper_class]}>
        ...
        >
          <.icon :if={!is_nil(item[:icon])} name={item[:icon]} class={["breadcrumb-icon", item[:icon_class]]} />
          <.link :if={!is_nil(item[:link])} navigate={item[:link]} title={item[:tile]} class={item[:link_class]}>{render_slot(item)}</.link>
